...
def plot(X, U):
    X = X/1E3 # km conversion for display
    x, y, z    = X[2, :], X[1, :], X[0, :]
    ...
    ax1.set_title('Control History', fontsize=9)
    ax1.set_ylabel(r'Fx ($m/s^2$)', fontsize=9)
    plt.setp(ax1.get_xticklabels(), visible=False)

    ax2 = fig.add_subplot(gs[1, 0], sharex=ax1)
    ax2.plot(uy, '-b', linewidth=0.8)
    ax2.set_ylabel(r'Fy ($m/s^2$)', fontsize=9)
    plt.setp(ax2.get_xticklabels(), visible=False)

    ax3 = fig.add_subplot(gs[2, 0], sharex=ax1)
    ax3.plot(uz, '-b', linewidth=0.8)   
    ax3.set_ylabel(r'Fz ($m/s^2$)', fontsize=9)
    ...
    ax4.scatter(xi, yi, zi, color='green') # , s=100
    ax4.scatter(xf, yf, zf, color='red') # , s=100    
